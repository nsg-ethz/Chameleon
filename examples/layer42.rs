...
const TOPO: TopologyZoo = TopologyZoo::Layer42;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ny = net.get_router_id("NewYorkCity")?;
    let la = net.get_router_id("LosAngeles")?;
    let ch = net.get_router_id("Chicago")?;
    net.build_external_routers(|_, _| vec![se, ny, la], ())?;
    let e_se = net.get_router_id("Seattle_ext_6")?;
    let e_ny = net.get_router_id("NewYorkCity_ext_7")?;
    let e_la = net.get_router_id("LosAngeles_ext_8")?;
    net.build_link_weights(uniform_integer_link_weight, (10, 100))?;
    net.build_ibgp_route_reflection(|_, _| vec![ch], ())?;
    ...
    {
        atomic_bgp::export_web(&net, &spec, decomposition.clone(), "layer42")?;
    }
